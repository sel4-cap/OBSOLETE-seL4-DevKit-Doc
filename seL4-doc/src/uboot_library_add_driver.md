...
1. The source file declaring U-Boot commands related to the driver class. For example, U-Boot commands related to GPIO are stored in the `uboot/cms/gpio.c` file.
...
The `plat_driver_data.h` and `plat_driver_data.c` files are used to create arrays of objects, such as drivers or commands, that U-Boot normally stores in dedicated linker sections (see [documentation on linker lists for details](uboot_driver_library.md#linker-lists)). The objects are declared in U-Boot source file via macros of the form `U_BOOT_<object_type> = { ... };`. All objects that the library needs to access must to declared and enumerated in the `plat_driver_data.h` and added to the `driver_data` structure within `plat_driver_data.c`.
...
Each declaration is of the form `extern struct <type> _u_boot_<class>__<name>` to match the declaration generated by the expansion of the `U_BOOT_<object_Type>` macro:

- `<name>` is the parameter provided to the macro that declared the object, e.g. a driver is declared with `U_BOOT_DRIVER(<name>)`.
- `<type>` and `<class>` directly correspond to type of object. Worked examples of the correct format for each object type are provided in the Avnet MaaXBoard's `plat_driver_data.h`. Alternatively the format for each type of object can be determined from the expansion of the macro for that object type, e.g. for a driver see the expansion of `#define U_BOOT_DRIVER`.

### `plat_driver_data.c`

Within the platform's `include/plat/<platform>/plat_driver_data.c` file the objects declared in the `plat_driver_data.h` file need to be added to the `driver_data` global structure. A worked example covering multiple object types is provided in the Avnet MaaXBoard's `plat_driver_datach` file.
...
1. Ensure seL4 permits access to the necessary devices from the platform's device tree; this is performed through the configuration of the application's CAmkES project file. For a worked example see the  `uboot-driver-example.camkes` file from the `uboot-driver-example` test application.
