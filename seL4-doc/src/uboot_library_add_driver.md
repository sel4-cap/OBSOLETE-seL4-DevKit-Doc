...
Each declaration is of the form `extern struct <type> _u_boot_<class>__<name>` where:
...
Care needs to be taken to ensure that the count of each type of object (declared within the `plat_driver_data.h`) is correctly maintained.

### `plat_driver_data.c`

Within the platform's `include/plat/<platform>/plat_driver_data.c` file the newly declared objects (e.g. driver, uclass driver, commands, etc) need to be added to the `driver_data` global structure. A worked example covering multiple object types is provided in the Avnet MaaXBoard's `plat_driver_datach` file.
